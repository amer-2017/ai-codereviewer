...
export async function runReview() {
  ...
  if (!diff) {
    ...
    // حتى لو ما في diff، بنستمر للـ PoC
  }
  ...
  // ────────────────────────────────────────────────
  // PoC: نستدعي analyzeCode دائمًا، حتى لو filteredDiff فارغة
  core.info("PoC: Calling analyzeCode regardless of filteredDiff count");
  const comments = await analyzeCode(filteredDiff, prDetails);
  core.info(`analyzeCode returned ${comments?.length || 0} comments`);
  // ────────────────────────────────────────────────
